    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="py-20 px-6 text-center bg-gradient-to-b from-indigo-50 to-white">
        ...
        <p className="text-lg text-slate-600 max-w-xl mx-auto">
          다양한 콘텐츠를 즐기고 나만의 결과를 확인해보세요. <br />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {testCards.map((test) => (
            <Link href={test.path} key={test.id} className="group">
              <div className={`h-full p-8 rounded-[2rem] transition-all duration-300 transform group-hover:-translate-y-2 group-hover:shadow-xl ${test.color} border border-transparent group-hover:border-indigo-200 flex flex-col`}>
